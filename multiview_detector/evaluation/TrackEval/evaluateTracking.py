...
def evaluationTracking_py(res_fpath, gt_fpath, dataset):
    ...
    for t, (gt_ids_t, tracker_ids_t) in enumerate(zip(gtRaw['gt_ids'], detRaw['tracker_ids'])):
    ...
    res['CLR_Frames'] = detRaw['num_timesteps']

    # Calculate final CLEAR scores
    res = _compute_final_fields(res)
    return res['CLR_F1'], res['MOTA'], res['MOTP']
