...
from multiview_detector.models.attn_module import (CBAM, ExpandedChannelGate,
                                                   ExpandedSpatialGate)
...
class GLAMSHOT(nn.Module):
    ...
    def warp_perspective(self, img_feature_all, proj_mats):
        warped_feat = 0
        channel_select = self.channel_attn(img_feature_all)  # [b*n,d,h,w]
        spatial_select = self.spatial_attn(img_feature_all)  # [b*n,d,h,w]
        for i in range(self.depth_scales):
            in_feat = (
                img_feature_all
                * channel_select[:, :, :, :, i]
                * spatial_select[:, i][:, None]
            )
