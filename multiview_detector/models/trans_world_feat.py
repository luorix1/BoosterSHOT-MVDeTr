...
from multiview_detector.models.deformable_transformer import (
...
from multiview_detector.models.transformer import (TransformerEncoder,
                                                   TransformerEncoderLayer)
from multiview_detector.utils.image_utils import array2heatmap


def create_pos_embedding(
    img_size, num_pos_feats=64, temperature=10000, normalize=True, scale=None
):
...
class TransformerWorldFeat(nn.Module):
    def __init__(
        ...
        self.downsample = nn.Sequential(
            nn.Conv2d(base_dim * num_cam, hidden_dim, 3, 2, 1),
            nn.ReLU(),
            nn.Conv2d(hidden_dim, hidden_dim, 3, 2, 1),
            nn.ReLU(),
        )

        self.pos_embedding = create_pos_embedding(
            np.ceil(np.array(Rworld_shape) / 4).astype(int), hidden_dim // 2
        )
        ...
        self.upsample = nn.Sequential(
            nn.Upsample(
                np.ceil(np.array(Rworld_shape) / 2).astype(int).tolist(),
                mode="bilinear",
            ),
    ...
    def forward(self, x, visualize=False):
        ...
        # H*W,B,C*N
        pos_embedding = (
            self.pos_embedding.repeat(B, 1, 1, 1)
            .flatten(2)
            .permute(2, 0, 1)
            .to(x.device)
        )
...
class DeformTransWorldFeat(nn.Module):
    ...
    def forward(self, x, visualize=False):
        ...
        lvl_pos_embed_flatten = (
            self.pos_embedding.to(x.device).flatten(2).transpose(1, 2).unsqueeze(1)
            + self.lvl_embedding.view([B, N, 1, C])
        ).view([B, N * H * W, C])
        spatial_shapes = torch.as_tensor(
            np.array([[H, W]] * N), dtype=torch.long, device=x.device
        )
        ...
        #         plt.show()
        merged_feat = self.merge_linear(
            memory.view(B, N, H, W, C)
            .permute(0, 1, 4, 2, 3)
            .contiguous()
            .view(B, N * C, H, W)
        )
...
class DeformTransWorldFeat_aio(nn.Module):
    def __init__(
        ...
        self.merge = nn.Sequential(
            nn.Conv2d(base_dim * num_cam, hidden_dim, 1),
            nn.ReLU(),
        )
        encoder_layer = DeformableTransformerEncoderLayer(
            hidden_dim, dim_feedforward, n_levels=1, n_heads=nhead
        )
    ...
    def forward(self, x, visualize=False):
        ...
        B, C, H, W = x.shape
        src_flatten = (
            x.view(B, C, H, W).permute(0, 2, 3, 1).contiguous().view([B, H * W, C])
        )
        spatial_shapes = torch.as_tensor(
            np.array([[H, W]]), dtype=torch.long, device=x.device
        )
        level_start_index = torch.cat(
            (spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1])
        )
        ...
        merged_feat = (
            memory.view(B, H, W, C).permute(0, 3, 1, 2).contiguous().view(B, C, H, W)
        )
