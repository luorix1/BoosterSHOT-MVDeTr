...
from multiview_detector.models.conv_world_feat import (ConvWorldFeat,
                                                       DeformConvWorldFeat)
from multiview_detector.models.gate_module import GateModule
from multiview_detector.models.resnet import resnet18
from multiview_detector.models.trans_world_feat import (DeformTransWorldFeat,
...
from multiview_detector.utils.projection import (
    get_worldcoord_from_imgcoord_mat, project_2d_points)
...
class CGSHOT(nn.Module):
    def __init__(
        ...
        if bottleneck_dim:
            self.bottleneck = nn.Sequential(
                nn.Conv2d(base_dim, bottleneck_dim, 3, padding=2, dilation=2),
                nn.ReLU(),
                nn.Conv2d(bottleneck_dim, bottleneck_dim, 3, padding=2, dilation=2),
            )
        ...
        # img heads
        self.img_heatmap = nn.Sequential(
            nn.Conv2d(base_dim, 64, 1), nn.ReLU(), nn.Conv2d(64, 1, 1, bias=False)
        )
        ...
        # Channel Gate (Conditional Channel Gated Networks for Task-Aware Continual Learning)
        self.channel_gate = nn.ModuleDict(
            {f"{i}": GateModule(base_dim, base_dim) for i in range(self.depth_scales)}
        )
        ...
        # world heads
        self.world_heatmap = nn.Conv2d(
            base_dim, 1, 3, padding=4, dilation=4, bias=False
        )
    ...
    def warp_perspective(self, img_feature_all, proj_mats):
        ...
        for i in range(self.depth_scales):
            in_feat = (
                self.channel_gate[f"{i}"](img_feature_all).unsqueeze(-1).unsqueeze(-1)
                * img_feature_all
            )
