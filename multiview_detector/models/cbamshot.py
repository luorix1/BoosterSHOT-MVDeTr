...
class CBAMSHOT(nn.Module):
    def __init__(self, dataset, arch='resnet18', world_feat_arch='conv',
        ...
        # Multiple CBAM modules, each corresponding to a single homography
        self.cbam_attn = nn.ModuleDict({f'{i}': CBAM(base_dim) for i in range(self.depth_scales)})
    ...
    def warp_perspective(self, img_feature_all, proj_mats):
        warped_feat = 0
        for i in range(self.depth_scales):
            in_feat = self.cbam_attn[f'{i}'](img_feature_all)
