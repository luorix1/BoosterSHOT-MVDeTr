...
class FrozenAttnChannelCutoff(nn.Module):
    def __init__(
        self,
        dataset,
        arch="resnet18",
        ...
        self.frozen_list = frozen_list if frozen_list else None
        ...
        if self.frozen_list is not None:
            for index in self.frozen_list:
                self.spatial_attn[f"{index}"] = nn.Identity()
