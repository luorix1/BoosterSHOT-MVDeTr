...
class MVDeTr(nn.Module):
    def __init__(
        ...
        self.proj_mats = torch.stack(
            [
                torch.from_numpy(
                    Rworldgrid_from_worldcoord_mat @ worldcoord_from_imgcoord_mats[cam]
                )
                for cam in range(dataset.num_cam)
            ]
        )
        ...
        if world_feat_arch == "conv":
            ...
            self.world_feat = DeformTransWorldFeat_aio(
                dataset.num_cam, dataset.Rworld_shape, base_dim
            )
        else:
            raise Exception
    ...
    def forward(
        ...
        proj_mats = (
            self.proj_mats.repeat(B, 1, 1, 1).view(B * N, 3, 3).float()
            @ imgcoord_from_Rimggrid_mat
        )
        ...
        if visualize:
            ...
            proj_imgs = kornia.geometry.warp_perspective(
                T.Resize(self.Rimg_shape)(imgs),
                proj_mats.to(imgs.device),
                self.Rworld_shape,
        ...
        world_feat = kornia.geometry.warp_perspective(
            imgs_feat, proj_mats.to(imgs.device), self.Rworld_shape
        ).view(B, N, C, H, W)
        if visualize:
            for cam in range(N):
                visualize_img = array2heatmap(
