...
from multiview_detector.models.cbam_resnet import ResidualNet
from multiview_detector.models.mobilenet import MobileNetV2
...
from multiview_detector.models.se_resnet import se_resnet18
from multiview_detector.models.trans_world_feat import (
    DeformTransWorldFeat,
    TransformerWorldFeat,
)
from multiview_detector.utils.image_utils import array2heatmap, img_color_denormalize
...
def output_head(in_dim, feat_dim, out_dim):
    if feat_dim:
        fc = nn.Sequential(
            nn.Conv2d(in_dim, feat_dim, 3, padding=1),
            nn.ReLU(),
...
def create_reference_map(dataset, n_points=4, downsample=2, visualize=False):
    ...
    world_zoom_mat = np.diag(
        [dataset.world_reduce * downsample, dataset.world_reduce * downsample, 1]
    )
    ...
    for cam in range(dataset.num_cam):
        mat_0 = Rworldgrid_from_worldcoord_mat @ get_worldcoord_from_imgcoord_mat(
            dataset.base.intrinsic_matrices[cam], dataset.base.extrinsic_matrices[cam]
        )
        ...
        if visualize:
            ...
            ax.streamplot(
                ref_x.numpy(), ref_y.numpy(), field_x.numpy(), field_y.numpy()
            )
...
class AttnChannelCutoff(nn.Module):
    def __init__(
        ...
        for i in range(self.depth_scales):
            proj_mats[i] = [
                torch.from_numpy(
                    Rworldgrid_from_worldcoord_mat @ worldcoord_from_imgcoord_mats[i][j]
                )
                for j in range(dataset.num_cam)
            ]
        ...
        for i in range(self.depth_scales):
            ...
            proj_mats[i] = nn.Parameter(
                proj.view([-1, 3, 3]), requires_grad=False
            ).cuda()
        ...
        if arch == "vgg11":
            ...
            self.base = nn.Sequential(
                *list(MobileNetV2(n_classes=10).children())[:-2],
                # nn.Upsample(scale_factor=4, mode="bilinear"),
            )
            base_dim = 512
        elif arch == "cbam-resnet18":
            self.base = ResidualNet(
            ...
            base_dim = 512
        elif arch == "se-resnet18":
            # self.base = nn.Sequential(
            #     *list(
            #         timm.create_model("legacy_seresnet18", pretrained=True).children()
            #     )[:-2]
            # )
            self.base = nn.Sequential(
                *list(se_resnet18(pretrained=False).children())[:-2]
            )
            base_dim = 512
        elif arch == "vit":
            self.base = nn.Sequential(
        ...
        if bottleneck_dim:
            ...
            self.bottleneck = nn.Sequential(
                nn.Conv2d(base_dim, bottleneck_dim, 1), nn.Dropout2d(dropout)
            )
        ...
        # img heads
        # self.img_heatmap = nn.Sequential(nn.Conv2d(base_dim, 64, 1), nn.ReLU(), nn.Conv2d(64, 1, 1))
        ...
        if self.use_head:
            # NOTE: newly added img head regression task
            self.img_heatmap_h = output_head(base_dim, outfeat_dim, 1)
            self.img_offset_h = output_head(base_dim, outfeat_dim, 2)
            self.img_wh_h = output_head(base_dim, outfeat_dim, 2)
        ...
        self.spatial_attn = nn.ModuleDict(
            {f"{i}": SpatialGate() for i in range(self.depth_scales)}
        )
        self.cutoff = CutoffModule(base_dim, self.depth_scales)

        # world feat
        if world_feat_arch == "conv":
            self.world_feat = ConvWorldFeat(
                dataset.num_cam, dataset.Rworld_shape, base_dim
            )
        elif world_feat_arch == "trans":
            self.world_feat = TransformerWorldFeat(
                dataset.num_cam, dataset.Rworld_shape, base_dim
            )
        elif world_feat_arch == "deform_conv":
            self.world_feat = DeformConvWorldFeat(
                dataset.num_cam, dataset.Rworld_shape, base_dim
            )
        elif world_feat_arch == "deform_trans":
            n_points = 4
            reference_points = create_reference_map(dataset, n_points).repeat(
                [dataset.num_cam, 1, 1, 1]
            )
            self.world_feat = DeformTransWorldFeat(
                dataset.num_cam,
                dataset.Rworld_shape,
                base_dim,
                n_points=n_points,
                stride=2,
        ...
        self.feat_before_concat = nn.Conv2d(
            base_dim, base_dim, 3, groups=self.depth_scales, padding=1
        )

        # world heads
        # self.world_heatmap = nn.Conv2d(base_dim, 1, 3, padding=4, dilation=4)
        ...
        # newly added head regression task
        if self.use_head:
            self.img_heatmap_h[-1].bias.data.fill_(-2.19)
            fill_fc_weights(self.img_offset_h)
            fill_fc_weights(self.img_wh_h)
        pass
    ...
    def forward(
        self,
        imgs,
        M=torch.tensor(np.identity(3), dtype=torch.float32)
        .unsqueeze(0)
        .repeat(7, 1, 1),
        logdir=None,
        visualize=False,
        ...
        proj_mats = [
            self.proj_mats[i].repeat(B, 1, 1, 1).view(B * N, 3, 3).float()
            @ imgcoord_from_Rimggrid_mat.cuda()
            for i in range(self.depth_scales)
        ]
        ...
        if visualize:
            ...
            for cam in range(N):
                ...
                # plt.show()
            for i in range(self.depth_scales):
                proj_imgs = kornia.geometry.warp_perspective(
                    T.Resize(self.Rimg_shape)(imgs), proj_mats[i], self.Rworld_shape
                ).view(B, N, 3, self.Rworld_shape[0], self.Rworld_shape[1])
                for cam in range(N):
                    visualize_img = T.ToPILImage()(denorm(proj_imgs.detach())[0, cam])
                    visualize_img.save(
                        os.path.join(logdir, f"imgs/augimgproj{cam + 1}.png")
                    )
                    # plt.imshow(visualize_img)
                    # plt.show()
        ...
        if visualize:
            for cam in range(N):
                visualize_img = array2heatmap(
                    torch.norm(imgs_feat[cam * B].detach(), dim=0).cpu()
                )
                visualize_img.save(
                    os.path.join(logdir, f"imgs/augimgfeat{cam + 1}.png")
                )
                # plt.imshow(visualize_img)
                # plt.show()
        ...
        if self.use_head:
            # newly added head regression task
            imgs_heatmap_h = self.img_heatmap_h(imgs_feat)
            imgs_offset_h = self.img_offset_h(imgs_feat)
            imgs_wh_h = self.img_wh_h(imgs_feat)
        ...
        if visualize:
            for cam in range(N):
                visualize_img = array2heatmap(
                    torch.norm(world_feat[0, cam].detach(), dim=0).cpu()
                )
        ...
        if visualize:
            visualize_img = array2heatmap(
                torch.norm(world_feat[0].detach(), dim=0).cpu()
            )
            visualize_img.save(os.path.join(logdir, f"imgs/worldfeatall.png"))
            # plt.imshow(visualize_img)
            # plt.show()
            visualize_img = array2heatmap(
                torch.sigmoid(world_heatmap.detach())[0, 0].cpu()
            )
        ...
        if self.use_head:
            return (
                ...
                (imgs_heatmap, imgs_offset, imgs_wh),
                (imgs_heatmap_h, imgs_offset_h, imgs_wh_h),
            )
        else:
...
def test():
    ...
    dataset = frameDataset(
        Wildtrack(os.path.expanduser("/workspace/Data/Wildtrack")),
        train=False,
        augmentation=False,
    )
    ...
    imgs = imgs.cuda()
    (world_heatmap, world_offset), (imgs_heatmap, imgs_offset, imgs_wh) = model(
        imgs, affine_mats
    )
