...
class ChannelCutoff(nn.Module):
    ...
    def warp_perspective(self, img_feature_all, proj_mats):
        ...
        # Channel-wise split
        attn_values = self.channel_attn(img_feature_all)
        values, indices = torch.topk(attn_values, img_feature_all.shape[1] // self.depth_scales, dim=1)
        indices = indices.squeeze(-2).squeeze(-2)

        for i in range(self.depth_scales):
            chosen = indices[:, :, i]
            in_feat = torch.cat([torch.index_select(img_feature_all[j], 0, chosen[j]).unsqueeze(0) for j in range(self.num_cam)], dim=0)
