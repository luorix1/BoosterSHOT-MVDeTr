...
from multiview_detector.models.se_module import SELayer
...
class SESHOT(nn.Module):
    def __init__(
        ...
        self.se = nn.ModuleDict(
            {f"{i}": SELayer(base_dim, reduction=1) for i in range(self.depth_scales)}
        )
    ...
    def warp_perspective(self, img_feature_all, proj_mats):
        warped_feat = 0
        for i in range(self.depth_scales):
            in_feat = self.se[f"{i}"](img_feature_all)
