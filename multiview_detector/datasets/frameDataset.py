...
def get_gt(
    Rshape, x_s, y_s, w_s=None, h_s=None, v_s=None, reduce=4, top_k=100, kernel_size=4
):
    H, W = Rshape
...
class frameDataset(VisionDataset):
    def __init__(
        ...
        self.img_shape, self.worldgrid_shape = (
            base.img_shape,
            base.worldgrid_shape,
        )  # H,W; N_row,N_col
        self.world_kernel_size, self.img_kernel_size = (
            world_kernel_size,
            img_kernel_size,
        )
        ...
        self.transform = T.Compose(
            [
                ...
                # T.Resize((np.array(self.img_shape) * 8 // 12).tolist()),
            ]
        )
        ...
        self.Rworld_shape = list(
            map(lambda x: x // self.world_reduce, self.worldgrid_shape)
        )
        self.Rimg_shape = (
            np.ceil(np.array(self.img_shape) / self.img_reduce).astype(int).tolist()
        )
        ...
        self.imgs_region = kornia.geometry.warp_perspective(
            world_masks, self.img_from_world, self.img_shape, "nearest"
        )
        ...
        for fname in sorted(
            os.listdir(os.path.join(self.root, "annotations_positions"))
        ):
            frame = int(fname.split(".")[0])
            if frame in frame_range:
                num_frame += 1
                keep = (
                    np.mean(np.array(frame_range) < frame) < self.semi_supervised
                    if self.semi_supervised
                    else 1
                )
                with open(
                    os.path.join(self.root, "annotations_positions", fname)
                ) as json_file:
                    all_pedestrians = json.load(json_file)
                world_pts, world_pids = [], []
                img_bboxs, img_pids = [[] for _ in range(self.num_cam)], [
                ...
                if keep:
                    for pedestrian in all_pedestrians:
                        ...
                        for cam in range(self.num_cam):
                            if itemgetter("xmin", "ymin", "xmax", "ymax")(
                                pedestrian["views"][cam]
                            ) != (-1, -1, -1, -1):
                                img_bboxs[cam].append(
                                    itemgetter("xmin", "ymin", "xmax", "ymax")(
                                        pedestrian["views"][cam]
                                    )
                                )
                                img_pids[cam].append(
                                    self.pid_dict[pedestrian["personID"]]
                                )
                                num_imgs_bbox += 1
                ...
                for cam in range(self.num_cam):
                    ...
                    self.imgs_gt[frame][cam] = (
                        np.array(img_bboxs[cam]),
                        np.array(img_pids[cam]),
                    )
                self.keeps[frame] = keep

        print(
            f"all: pid: {len(self.pid_dict)}, frame: {num_frame}, keep ratio: {num_keep / num_all:.3f}\n"
    ...
    def get_world_imgs_trans(self, z=0):
        ...
        proj_mats = [
            Rworldgrid_from_worldcoord_mat
            @ worldcoord_from_imgcoord_mats[cam]
            @ self.base.img_xy_from_xy_mat
            for cam in range(self.num_cam)
        ]
        world_from_img = torch.tensor(np.stack(proj_mats))
        # img(xy)_from_worldgrid(xy)
        img_from_world = torch.tensor(
            np.stack([np.linalg.inv(proj_mat) for proj_mat in proj_mats])
        )
    ...
    def prepare_gt(self):
        ...
        for fname in sorted(
            ...
            for single_pedestrian in all_pedestrians:
                ...
                def is_in_cam(cam):
                    return not (
                        single_pedestrian["views"][cam]["xmin"] == -1
    ...
    def __getitem__(self, index, visualize=False):
        def plt_visualize():
            ...
            for bbox in img_bboxs:
                ...
                cv2.rectangle(
                    img0, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 255, 0), 2
                )
            plt.imshow(img0)
            plt.show()
        ...
        for cam in range(self.num_cam):
            ...
            img_w_s, img_h_s = (img_bboxs[:, 2] - img_bboxs[:, 0]), (
                img_bboxs[:, 3] - img_bboxs[:, 1]
            )
        ...
        imgs_gt = {
            key: torch.stack([img_gt[key] for img_gt in imgs_gt]) for key in imgs_gt[0]
        }
        if self.use_head:
            imgs_gt_h = {
                key: torch.stack([img_gt_h[key] for img_gt_h in imgs_gt_h])
                for key in imgs_gt_h[0]
            }
        # imgs_gt['heatmap_mask'] = self.imgs_region if self.keeps[frame] else torch.zeros_like(self.imgs_region)
        # imgs_gt['heatmap_mask'] = kornia.geometry.warp_perspective(imgs_gt['heatmap_mask'], affine_mats, self.img_shape,
        #                                                   align_corners=False)
        # imgs_gt['heatmap_mask'] = F.interpolate(imgs_gt['heatmap_mask'], self.Rimg_shape, mode='bilinear',
        #                                         align_corners=False).bool().float()
        drop, keep_cams = np.random.rand() < self.dropout, torch.ones(
            self.num_cam, dtype=torch.bool
        )
        if drop:
        ...
        if self.use_head:
            return imgs, world_gt, imgs_gt, imgs_gt_h, affine_mats, frame
        else:
...
def test(test_projection=False):
    ...
    if test_projection:
        ...
        for cam in range(dataset.num_cam):
            world_coords = get_worldcoord_from_imagecoord(
                image_coords.transpose(),
                dataset.base.intrinsic_matrices[cam],
                dataset.base.extrinsic_matrices[cam],
            )
            world_grids = (
                dataset.base.get_worldgrid_from_worldcoord(world_coords)
                .transpose()
                .reshape([H, W, 2])
            )
            ...
            for i in range(H):
                for j in range(W):
                    ...
                    if dataset.base.indexing == "xy":
                        if x in range(dataset.worldgrid_shape[1]) and y in range(
                            dataset.worldgrid_shape[0]
                        ):
                            world_grid_map[int(y), int(x)] += 1
                    else:
                        if x in range(dataset.worldgrid_shape[0]) and y in range(
                            dataset.worldgrid_shape[1]
                        ):
