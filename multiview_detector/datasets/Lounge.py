...
class Lounge(VisionDataset):
    def __init__(self, root):
        super().__init__(root)
        # image of shape C,H,W (C,N_row,N_col); xy indexging; x,y (w,h) (n_col,n_row)
        # Lounge has ij-indexing: H*W=480*1440, thus x (i) is \in [0,480), y (j) is \in [0,1440)
        # Lounge has in-consistent unit: centi-meter (cm) for calibration & pos annotation,
        self.__name__ = "Lounge"
        self.img_shape, self.worldgrid_shape = [1080, 1920], [
            480,
            1440,
        ]  # H,W; N_row,N_col
        self.num_cam, self.num_frame = 8, 2000
        # world x,y actually means i,j in Lounge, which correspond to h,w
        self.indexing = "ij"
...
def test():
    ...
    dataset = Lounge(
        os.path.expanduser("~/Data/Lounge"),
    )
