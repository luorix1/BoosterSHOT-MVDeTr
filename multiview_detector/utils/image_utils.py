...
def random_affine(
    ...
    # Translation
    T = np.eye(3)
    T[0, 2] = (random.random() * 2 - 1) * translate[
        0
    ] * width + border  # x translation (pixels)
    T[1, 2] = (random.random() * 2 - 1) * translate[
        1
    ] * height + border  # y translation (pixels)
    ...
    # warp points
    xy = np.ones((n * 4, 3))
    xy[:, :2] = bboxs[:, [0, 1, 2, 3, 0, 3, 2, 1]].reshape(
        n * 4, 2
    )  # x1y1, x2y2, x1y2, x2y1
...
def add_heatmap_to_image(heatmap, image):
    ...
    cam_result = np.uint8(heatmap * 0.7 + np.array(image) * 0.3)
