...
def main(args):
    ...
    for batch_idx, (_, _, _, _, frame) in enumerate(test_loader):
        ...
        res_map_grid = results[
            results[:, 0] == (frame.detach().cpu().numpy() - 1800) // 5 + 1, 1:
        ]

        for result in res_map_grid:
            ct = np.array(
                [
                    (result[2] + result[4]) / 2 / test_set.world_reduce,
                    (result[3] + result[5]) / test_set.world_reduce,
                ],
                dtype=np.float32,
            )
        ...
        plt.imsave(
            os.path.join("/workspace/MVDeTr_research/bev", f"{(batch_idx + 1):08}.png"),
            heatmap[0],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--res_fpath",
        help="Absolute path of parent directory",
