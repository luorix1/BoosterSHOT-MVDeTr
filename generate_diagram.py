...
def main(args):
    ...
    for batch_idx, (data, world_gt, imgs_gt, affine_mats, frame) in enumerate(
        ...
        for result in res_map_grid:
            ct = np.array(
                [result[0] / test_set.world_reduce, result[1] / test_set.world_reduce],
                dtype=np.float32,
            )
        ...
        subplt1.imshow(
            denormalize(imgs[0]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        )
        subplt2.imshow(
            denormalize(imgs[1]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        )
        subplt3.imshow(
            denormalize(imgs[2]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        )
        subplt5.imshow(
            denormalize(imgs[3]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        )
        subplt6.imshow(
            denormalize(imgs[4]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        )
        subplt7.imshow(
            denormalize(imgs[5]).detach().cpu().numpy().squeeze().transpose([1, 2, 0])
        )

        subplt4 = fig.add_subplot(244, title="output")
        subplt8 = fig.add_subplot(248, title="target")
        ...
        plt.subplots_adjust(
            wspace=0.3, hspace=0.05, top=0.9, bottom=0.1, left=0.1, right=0.9
        )
        plt.savefig(
            os.path.join(args.ckpt_path, f"diagrams/diagram_{batch_idx + 1}.png")
        )
