...
from multiview_detector.models.f_attnchannelcutoff import FrozenAttnChannelCutoff
from multiview_detector.models.mvdetr import MVDeTr
from multiview_detector.models.sashot import SASHOT
from multiview_detector.models.seshot import SESHOT
...
def main(args):
    ...
    if "lounge" in args.dataset:
    ...
    elif "wildtrack_60" in args.dataset:
        base = Wildtrack_60(os.path.expanduser("/workspace/Data/Wildtrack_old"))
    elif "wildtrack" in args.dataset:
        base = Wildtrack(os.path.expanduser("/workspace/Data/Wildtrack"))
    elif "multiviewx_hard" in args.dataset:
        base_train = MultiviewX_hard(os.path.expanduser("/workspace/Data/MultiviewX"))
        base_test = MultiviewX_hard_test(
            os.path.expanduser("/workspace/Data/MultiviewX")
        )
        ...
        raise Exception(
            "must choose from [lounge, retail, wildtrack, multiviewx, multiviewx_hard]"
        )

    if "multiviewx_hard" in args.dataset:
    ...
    if args.resume is None:
        logdir = (
            f'logs/{args.dataset}/{"debug_" if is_debug else ""}{"SS_" if args.semi_supervised else ""}'
    ...
    if args.task == "detection":
        ...
        elif args.model == "FrozenAttnChannelCutoff":
    ...
    if args.resume is None:
        for epoch in tqdm.tqdm(range(1, args.epochs + 1)):
            ...
            train_loss = trainer.train(
                epoch, train_loader, optimizer, scaler, scheduler
            )
            print("Testing...")
            test_loss, moda = trainer.test(
                epoch, test_loader, res_fpath, visualize=True
            )
            ...
            torch.save(
                model.state_dict(), os.path.join(logdir, "MultiviewDetector.pth")
            )
    else:
        model.load_state_dict(
            torch.load(f"logs/{args.dataset}/{args.resume}/MultiviewDetector.pth")
        )
        model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", default="detection", choices=["detection", "tracking"]
    )
    ...
    parser.add_argument(
        "--alpha", type=float, default=1.0, help="ratio for per view loss"
    )
    ...
    parser.add_argument(
        "-b", "--batch_size", type=int, default=1, help="input batch size for training"
    )
