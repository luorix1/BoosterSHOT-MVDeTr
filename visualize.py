...
def main(args):
    ...
    for cam in range(base.num_cam):
        ...
        x = np.arange(args.bottleneck_dim)
        y = np.arange(args.depth_scales)
        heatmap = axes.pcolormesh(x, y, heatmap[cam], vmin=0, vmax=1)
        # figure.colorbar()
        figure.savefig(os.path.join(args.ckpt_path, f'heatmap/selection_heatmap_{cam + 1}.png'))
